...
def run_nuclei_type_stat(pred_dir, true_dir, type_uid_list=None, exhaustive=True):
    ...
    """
    pred_instance_centers_folder = os.path.join(pred_dir, "instance_centers")
    gt_instance_centers_folder = os.path.join(true_dir, "instance_centers")
    pred_types_folder = os.path.join(pred_dir, "instance_types")
    gt_types_folder = os.path.join(true_dir, "instance_types")

    file_names = os.listdir(pred_instance_centers_folder)
    file_names.sort()  # ensure same order [1]
    ...
    for file_idx, filename in enumerate(file_names):
        true_centroid = np.load(os.path.join(gt_instance_centers_folder, filename))
        true_inst_type = np.load(os.path.join(gt_types_folder, filename))
        true_inst_type = true_inst_type[:, np.newaxis]
        ...
        pred_centroid = np.load(os.path.join(pred_instance_centers_folder, filename))
        pred_inst_type = np.load(os.path.join(pred_types_folder, filename))
        pred_inst_type = pred_inst_type[:, np.newaxis]
...
def run_nuclei_inst_stat(pred_dir, true_dir, print_img_stats=False):
    ...
    gt_segmentation_folder = os.path.join(true_dir, "seg_map")
    pred_segmentation_folder = os.path.join(pred_dir, "seg_map")

    file_names = os.listdir(gt_segmentation_folder)
    file_names.sort()  # ensure same order

    metrics = [[], [], [], [], [], []]
    for filename in file_names:
        true = np.load(os.path.join(gt_segmentation_folder, filename))
        true = true.astype("int32")

        pred = np.load(os.path.join(pred_segmentation_folder, filename))
    ...
    np.set_printoptions(formatter={"float": "{: 0.5f}".format})
    print("  dice_1,  fast aji,  ap,      sq,      pq,    fast_aji_plus")
    print(metrics_avg)
