...
def inference_detector(model, imgs, show=False):
    ...
    if not isinstance(imgs, list):
        if not show:
            ...
            return _inference_single_show(model, imgs, img_transform, device)
    else:
...
def _inference_single_show(model, img, img_transform, device):
    img = mmcv.imread(img)
    data = _prepare_data(img, img_transform, model.cfg, device)
    with torch.no_grad():
        result = model(return_loss=False, rescale=True, show=True, **data)
    return result
