...
from .utils import random_scale, to_tensor, contour_random_walk, fillInstance
...
class PanNuke_Seg_Dataset_gt(CustomDataset):
    ...
    def __init__(self,
        ...
        super(PanNuke_Seg_Dataset_gt, self).__init__(
        ...
        self.polar_coordinate = polar_coordinate  # 是否获取极角极径形式的gt
        ...
        self.heatmap_contour = heatmap_contour  # heatmap分支选的点是否是轮廓上的所有点,若为否则只有采样点
        ...
        self.polar_xy_represent = polar_xy_represent  # 在使用polar的gt时是否使用二维坐标
    ...
    def prepare_train_img(self, idx):
        ...
        if self.polar_coordinate:
            ...
            #         concat_regress_ranges)
            if self.polar_xy:
                if not self.sample_point_heatmap:
                    _labels, _bbox_targets, _mask_targets, _polar_targets = self.polar_target_single(
                        gt_bboxes, gt_masks, gt_labels, concat_points,
                        concat_regress_ranges)
                else:
                    _labels, _bbox_targets, _mask_targets, _polar_targets, _gt_sample_heatmap = self.polar_target_single(
                        gt_bboxes, gt_masks, gt_labels, concat_points,
                        concat_regress_ranges)
            else:
                if self.sample_point_heatmap:
    ...
    def polar_target_single(self, gt_bboxes, gt_masks, gt_labels, points,
        ...
        for i in range(gt_masks.shape[0]):
            ...
            if len(contours) > 1 and self.fill_instance:
                ...
                if self.center_fill_before:
                    ...
                    mask, cnt = fillInstance(mask, self.center_fill_before)
                    _, contour = self.get_single_centerpoint(mask)
                    #cv2.imwrite('./vis/mask_{}.jpg'.format(i), mask * 255)
                else:
                    # 以填充后的mask的质心为中心
                    #cv2.imwrite('./vis/mask_old_{}.jpg'.format(i), mask * 255)
                    mask = fillInstance(mask, self.center_fill_before)
        ...
        if self.polar_coordinate:
            ...
            if self.polar_xy:
                if not self.polar_xy_represent:
                    ...
                    polar_targets = torch.zeros(num_points, 36, 2).float()
        ...
        for p, id in zip(pos_inds, pos_mask_ids):
            ...
            if self.polar_xy:
                ...
                dists, _, start = self.get_36_coordinates(
                    gt_masks[id], x, y, mask_contour
                )  # 此处得到的点对应的极角是[0，...,350]，并找到轮廓上距离polarmask最近的点并作为起点
                # 使用极径形式表示polarmask的gt
                if not self.polar_xy_represent:
                    polar_targets[p] = dists
                else:
                    # 使用二维偏移的形式表示polarmask的gt
                    sin = torch.sin(self.angles)
                    cos = torch.cos(self.angles)

                    polar_targets[p, :,
                                  0] = dists * sin * self.normalize_factor
                    polar_targets[p, :,
                                  1] = dists * cos * self.normalize_factor
                    # # test
                    # polar_targets[p, :, 0] = dists * sin + x
                    # polar_targets[p, :, 1] = dists * cos + y
                ...
                mask_id = id.numpy().tolist()
                points_sample_gt = points_sample_gt_dict[
                    mask_id]  # 之前已经保存过每条轮廓需要的点
                mask_id_targets[p] = mask_id  # 保存每个实例对应的mask_id,方便后面取每一层的轮廓采样点
                # print('gt iou:', iou)
                if self.polar_coordinate:
            ...
            # # mask[hyu8u[:, 1].int().numpy().tolist(), points_sample_gt[:, u0].int().numpy().tolist(), :2] = 0
            # # mask = cv2.polylines(
            # #     mask,
            ...
            # mask = cv2.polylines(
            #     mask,
            #     polar_targets.numpy().astype(np.int32)[p, :4, :][None, :, :],
            #     True, (0, 255, 0), 2)
        ...
        # TODO: 对于在poalr_xy=True,heatmap_contour=False（即仅采样每条轮廓上的36个点）的情况还没有实现
        if self.sample_point_heatmap:
            # lvl_mask_ids = mask_id_targets.split([15360, 3840, 960, 240, 60],
            #                                      0)
            lvl_mask_ids = mask_id_targets.split(self.num_points_per_level, 0)
        ...
        if self.polar_coordinate:
            ...
            if self.polar_xy:
                if not self.sample_point_heatmap:
                    return labels, bbox_targets, points_targets, polar_targets  # 等间隔取点、等角度间隔取点
                else:
                    # 在同时取polar和xy的情况下，如果采点heatmap
                    return labels, bbox_targets, points_targets, polar_targets, points_sample_map_rescale_lvl
            if self.sample_point_heatmap:
