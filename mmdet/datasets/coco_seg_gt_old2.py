...
class Coco_Seg_Dataset_gt(CustomDataset):
    ...
    def __init__(self,
        ...
        self.heatmap_contour = heatmap_contour  # heatmap分支选的点是否是轮廓上的所有点
        if self.fixed_gt:
    ...
    def polar_target_single(self, gt_bboxes, gt_masks, gt_labels, points,
        ...
        lvl_mask_ids = mask_id_targets.split([15360, 3840, 960, 240, 60], 0)
        lvl_mask_ids = [
        ...
        for i in range(5):
            ...
            mask_points_rescale = torch.zeros(
                (int(mask.shape[0] / 8),
                 int(mask.shape[1] /
                     8)))  # 针对P3，确定生成的分类标签的图尺寸为下采样8倍 # TODO：如果要修改在其他尺寸上绘图
