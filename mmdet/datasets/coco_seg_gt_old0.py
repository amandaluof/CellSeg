...
class Coco_Seg_Dataset_gt(CustomDataset):
    ...
    def __init__(self,
        ...
        self.polar_coordiante = polar_coordinate
    ...
    def prepare_train_img(self, idx):
        ...
        # add by amd,获取360度的gt
        _labels, _bbox_targets, _mask_targets, _contour_all_points = self.polar_target_single(
            gt_bboxes, gt_masks, gt_labels, concat_points,
        ...
        data['_gt_contour_all_points'] = DC(_contour_all_points)
        # --------------------offline ray label generation-----------------------------
    ...
    def polar_target_single(self, gt_bboxes, gt_masks, gt_labels, points,
        ...
        if not self.fixed_gt:
            mask_targets = torch.zeros(num_points,
                                       36).float()  # 这个数组可能不是从0度开始的，而是90
        else:
        ...
        # add by amd
        if not self.fixed_gt:
            contour_all_points = torch.zeros(num_points,
                                             self.num_sample_points).float()
        else:
            contour_all_points = None
        ...
        for p, id in zip(pos_inds, pos_mask_ids):
            ...
            if not self.fixed_gt:
                dists, coords = self.get_36_coordinates(
                    x, y, pos_mask_contour,
                    self.num_sample_points)  ###对每一个中心求它的极径
                contour_all_points[p] = dists
                mask_targets[p] = dists[::int(
                    10 / (360 / self.num_sample_points)
                )]  ### mask_targets is get by same angle interval
            else:
                ...
                #     mask, (x, y), 1,
                #     (0, 0,
                #      255))  # 当前像素,正样本像素不一定在物体内，在物体很小的情况下，可能在物体外部(1.5stride）
                # mask = cv2.circle(
                #     mask, (mask_centers[p][id][1], mask_centers[p][id][0]), 1,
                #     (255, 0, 0))  # real mass center
                # mask = cv2.polylines(
                #     mask,
                #     points_sample_gt.numpy().astype(np.int32)[None, :, :],
                #     True, (0, 0, 255), 2)
                # mask = cv2.polylines(
                #     mask,
                #     points_sample_gt.numpy().astype(np.int32)[:, None, :],
        ...
        if not self.fixed_gt:
            return labels, bbox_targets, mask_targets, contour_all_points
        elif self.polar_coordiante:
    ...
    def get_36_coordinates(self, c_x, c_y, pos_mask_contour,
        ...
        assert (360 % num_sampling_points
                ) == 0, "360 is not multiple to num_sampling_points"
        ...
        # 生成36个角度
        new_coordinate = {}
        for i in range(0, 360, 360 // num_sampling_points):
            if i in angle:
                d = dist[angle == i].max()  # 取出角度为i的最大值
                new_coordinate[i] = d
            elif i + 1 in angle:
        ...
        distances = torch.zeros(num_sampling_points)

        for a in range(0, 360, 360 // num_sampling_points):
            if not a in new_coordinate.keys():
                new_coordinate[a] = torch.tensor(1e-6)
                distances[a // (360 // num_sampling_points)] = 1e-6
            else:
                distances[a //
                          (360 // num_sampling_points)] = new_coordinate[a]
        # for idx in range(36):
