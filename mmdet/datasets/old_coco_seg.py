...
class Coco_Seg_Dataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        self.regress_ranges = ((-1, 64), (64, 128),
    ...
    def polar_target_single(self, gt_bboxes, gt_masks, gt_labels, points, regress_ranges):
        ...
        gt_bboxes: 表示图中所有实例的bbox
        points: 特征图上所有的点(对应到原图的位置)
        '''
    ...
    def get_mask_sample_region(self, gt_bb, mask_center, strides, num_points_per, gt_xs, gt_ys, radius=1):
        ...
        for level, n_p in enumerate(num_points_per):
            ...
            # limit sample region in gt
            center_gt[beg:end, :, 0] = torch.where(
                xmin > gt_bb[beg:end, :, 0], xmin, gt_bb[beg:end, :, 0])
            center_gt[beg:end, :, 1] = torch.where(
                ymin > gt_bb[beg:end, :, 1], ymin, gt_bb[beg:end, :, 1])
            center_gt[beg:end, :, 2] = torch.where(
                xmax > gt_bb[beg:end, :, 2], gt_bb[beg:end, :, 2], xmax)
            center_gt[beg:end, :, 3] = torch.where(
                ymax > gt_bb[beg:end, :, 3], gt_bb[beg:end, :, 3], ymax)
            beg = end
    ...
    def get_single_centerpoint(self, mask):
        '''
        input: 掩码
        output: 中心、掩码中的所有轮廓(按照面积大小排序)
        '''
        contour, _ = cv2.findContours(
            mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
        contour.sort(key=lambda x: cv2.contourArea(
            x), reverse=True)  # only save the biggest one
