...
class Scale_list(nn.Module):
    def __init__(self, scale=[1.0, 1.0]):
        super(Scale_list, self).__init__()
        self.scale = nn.Parameter(
            torch.tensor(scale, dtype=torch.float)[None, :, None, None,
                                                   None])  # 可训练参数
...
class Scale_channel(nn.Module):
    def __init__(self, scale=1.0, channels=1):
        ...
        self.scale = nn.Parameter(
            torch.ones(channels, dtype=torch.float) * scale)
