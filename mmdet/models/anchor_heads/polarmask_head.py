...
class PolarMask_Head(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_mask=dict(type='MaskIOULoss_v0'),
    ...
    def get_bboxes_single(self,
                          ...
                          rescale=False):
        '''
        cls_scores: [(80,96,160),(80,48,80)...(80,6,10)], all images' cls_score sorted by level
        bbox_preds: [(4,96,160),(4,48,80)...(4,6,10)], all images' bbox sorted by level
        mask_preds: [(36,96,160),(36,48,80)...(36,6,10)], all images' mask sorted by level
        mlvl_points: all feature map points' coordinates [(15360,2),(3840,2),(960,2),(240,2),(60,2)]
        '''
        ...
        for cls_score, bbox_pred, mask_pred, centerness, points in zip(
            ...
            # select topk bbox at most per level
        ...
        mlvl_bboxes = torch.cat(mlvl_bboxes)  # (3260,4)
        mlvl_masks = torch.cat(mlvl_masks)  # (3260,2,36)
        # add by amd
        mlvl_center = torch.cat(mlvl_center)  #(3260,2)

        if rescale:
            _mlvl_bboxes = mlvl_bboxes / mlvl_bboxes.new_tensor(
                scale_factor)  # rescale bbox to macth orignal shape
            try:
                scale_factor = torch.Tensor(scale_factor)[:2].cuda().unsqueeze(
                    1).repeat(1, 36)  # only two dimensions are needed
