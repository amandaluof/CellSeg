...
class PolarMask_Deviation_Head(nn.Module):
    def __init__(self,
                 num_classes,
                 in_channels,
                 feat_channels=256,
                 ...
                 loss_mask=dict(type='MaskIOULoss', deviation=True),
                 loss_deviation_sum=dict({}),
                 loss_deviation_reg=dict({}),
                 loss_centerness=dict(type='CrossEntropyLoss',
                                      use_sigmoid=True,
                                      loss_weight=1.0),
                 loss_radius_smooth=dict({}),
                 conv_cfg=None,
                 norm_cfg=dict(type='GN', num_groups=32, requires_grad=True),
                 num_sample_points=360):
        super(PolarMask_Deviation_Head, self).__init__()
        ...
        # add by amd
        if len(loss_deviation_sum) == 0:
            self.deviation_sum = False
        else:
            self.deviation_sum = True
            self.loss_deviation_sum = build_loss(loss_deviation_sum)

        if len(loss_deviation_reg) == 0:
            self.deviation_reg = False
        else:
            self.deviation_reg = True
            self.loss_deviation_reg = build_loss(loss_deviation_reg)

        if len(loss_radius_smooth) == 0:
            self.radius_smooth = False
        else:
            self.radius_smooth = True
            self.loss_radius_smooth = build_loss(loss_radius_smooth)
    ...
    def _init_layers(self):
        ...
        self.polar_angle = nn.Conv2d(self.feat_channels, 36, 3,
                                     padding=1)  # add by amd, 预测相邻极角的差值
    ...
    def forward_single(self, x, scale_bbox, scale_mask):
        ...
        # v0,错误加上了指数
        #deviation_pred = self.polar_angle(angle_feat).float().exp()
        # v1
        #deviation_pred = self.polar_angle(angle_feat).float()# 偏差可正可负
        # v2，求在正负5内的偏差
        # deviation_pred = (self.polar_angle(angle_feat).float().sigmoid() * 10 -
        #                   5) / 180 * math.pi
        deviation_pred = self.polar_angle(angle_feat).float() / 180 * math.pi

        # TODO：当前输出如何用于验证阶段？？

        return cls_score, bbox_pred, centerness, mask_pred, deviation_pred

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'mask_preds',
                          'deviation_preds', 'centernesses'))
    def loss(self,
             cls_scores,
             bbox_preds,
             centernesses,
        ...
        deviation_preds: 预测所有level的像素的36条射线的角度与10度的偏移[(batch,36,H1,W1),(batch,36,H2,W2)...(batch,36,H5,W5)]
        ...
        assert len(cls_scores) == len(bbox_preds) == len(centernesses) == len(
            mask_preds) == len(deviation_preds)
        ...
        # if debug:
        #     faked_deviation = [
        #         torch.zeros(deviation.shape, requires_grad=True).cuda()
        #         for deviation in deviation_preds
        #     ]
        #     deviation_preds = faked_deviation
        #     print('deviation is faked, only for debug')

        # target
        labels, bbox_targets, mask_targets, contour_all_points = self.polar_target(
        ...
        flatten_deviation_preds = [
            deviation_pred.permute(0, 2, 3, 1).reshape(-1, 36)
            for deviation_pred in deviation_preds
        ]
        ...
        flatten_deviation_preds = torch.cat(
            flatten_deviation_preds)  # [num_pixel, 36]
        ...
        flatten_contour_all_points = torch.cat(
            contour_all_points)  # [num_pixel, 360]

        # 在10度的基础上，根据pre_deviation得到delta theta, 再由此累加得到累计角度theta
        # 然后从flatten_contour_all_points中选出对应的位置作为极径的真值
        # 可以根据theta对应的值选出相邻两个角度的极径，进行线性插值
        pred_deviation = flatten_deviation_preds.clone().detach()
        pred_deviation = (pred_deviation / np.pi) * 180  # 将原有的0-2pi转换为0-360
        pred_delta_angle_init = torch.ones(
            pred_deviation.shape, requires_grad=False) * 10  # 按照10度的角度间隔为基准
        pred_delta_angle = pred_delta_angle_init.cuda() + pred_deviation
        pred_inds = torch.cumsum(pred_delta_angle, dim=1)  #[num_pixels,36]
        pred_inds = pred_inds.clamp(min=0, max=359)

        # pred_inds_left = torch.floor(pred_inds).long()
        # # 从360个点中取出36个点的值，对于小数，按照相邻2点线性插值
        # x_inds = torch.Tensor(range(len(pred_inds))).expand((pred_inds.size(
        # )[1], len(pred_inds))).T.flatten().cuda(
        # )
        # y_inds_left = pred_inds_left.flatten()
        # y_inds_right = y_inds_left + 1

        interval = 360 / self.num_sample_points
        pred_inds_left = (pred_inds //
                          interval).long()  # [num_pixels,36],计算对应下标
        x_inds = torch.Tensor(range(len(pred_inds))).expand((pred_inds.size(
        )[1], len(pred_inds))).T.flatten().cuda(
        )  # 计算面积时采样36个点，[num_pixels*36,1]为后面的元素获取做准备，将每一行的行号重复36次[0,0,0..0,...,40919,40919...40919]
        y_inds_left = pred_inds_left.flatten()
        y_inds_right = y_inds_left + 1

        y_inds_left = y_inds_left.clamp(
            min=0, max=self.num_sample_points -
            1)  # 取的下标应该在[0,num_sample_points）之间 # [num_pixels*36]
        y_inds_right = y_inds_right.clamp(min=0,
                                          max=self.num_sample_points -
                                          1)  # [num_pixels*36]
        weight_right_contour = (
            pred_inds.flatten() - y_inds_left *
            interval) / interval  #  # [num_pixels*36],分配左右权重，离左边的点近则右边的点权重大，
        weight_left_contour = 1 - weight_right_contour

        flatten_contour_targets = weight_left_contour * flatten_contour_all_points[
            x_inds.long(), y_inds_left.long(
            )] + weight_right_contour * flatten_contour_all_points[
                x_inds.long(), y_inds_right.long()]
        flatten_contour_targets = flatten_contour_targets.reshape(
            -1, 36)  # [num_pixel, 36]
        ...
        # print('predicted centerness:{}'.format(pos_centerness))
        pos_mask_preds = flatten_mask_preds[pos_inds]
        pos_deviation_preds = flatten_deviation_preds[pos_inds]

        if num_pos > 0:
            pos_bbox_targets = flatten_bbox_targets[pos_inds]
            pos_mask_targets = flatten_contour_targets[pos_inds]
            ...
            # 计算极径和极角的联合损失
            if (pos_mask_targets == 0).all():
                import pdb
                pdb.set_trace()

            loss_mask = self.loss_mask(pos_deviation_preds,
                                       pos_mask_preds,
                                       pos_mask_targets,
                                       weight=pos_centerness_targets,
                                       avg_factor=pos_centerness_targets.sum())
            ...
            # 约束预测的deviation在正负5/180*pi之间,即deviation的绝对值小于等于5
            if self.deviation_reg:
                deviation_regular = torch.ones_like(
                    pos_deviation_preds,
                    requires_grad=False) * 5 / 180 * math.pi
                deviation_regular = deviation_regular.cuda()
                loss_deviation_reg = self.loss_deviation_reg(
                    pos_deviation_preds,
                    deviation_regular,
                    weight=pos_centerness_targets,
                    avg_factor=pos_centerness_targets.sum())
                loss_dict['loss_deviation_reg'] = loss_deviation_reg

            # 约束预测的deviation累计偏差为0
            if self.deviation_sum:
                deviation_sum_regular = torch.zeros_like(
                    pos_deviation_preds.sum(axis=1), requires_grad=False)
                deviation_sum_regular = deviation_sum_regular.cuda()
                loss_deviation_sum = self.loss_deviation_sum(
                    pos_deviation_preds,
                    deviation_sum_regular,
                    weight=pos_centerness_targets,
                    avg_factor=pos_centerness_targets.sum())  # 平滑l1损失
                loss_dict['loss_deviation_sum'] = loss_deviation_sum

            # 极径平滑损失
            if self.radius_smooth:
                loss_radius_smooth = self.loss_radius_smooth(
                    pos_mask_preds, None)
                loss_dict['loss_radius_smooth'] = loss_radius_smooth
            ...
            loss_dict['loss_centerness'] = loss_centerness
            if self.deviation:
                loss_deviation_sum = pos_deviation_preds.sum()
                loss_dict['loss_deviation_sum'] = loss_deviation_sum

            if self.radius_smooth:
                loss_radius_smooth = torch.zeros(pos_mask_preds.sum().shape,
                                                 requires_grad=True)
                loss_dict['loss_radius_smooth'] = loss_radius_smooth
            # TODO：图像中无实例时，IOU LOSS的计算不需要极角？？
        ...
        if debug:
            return loss_dict, pos_points, pos_mask_targets, pos_deviation_preds, flatten_contour_all_points[
                pos_inds], pos_inds
        else:
            return loss_dict
    ...
    def polar_target(self, points, extra_data):
        ...
        labels_list, bbox_targets_list, mask_targets_list, contour_all_points_list = extra_data.values(
        )
        ...
        contour_all_points_list = [
            contour_all_points.split(num_points, 0)
            for contour_all_points in contour_all_points_list
        ]
        ...
        for i in range(num_levels):
            ...
            concat_lvl_contour_all_points.append(
                torch.cat([
                    contour_all_points[i]
                    for contour_all_points in contour_all_points_list
                ]))

        return concat_lvl_labels, concat_lvl_bbox_targets, concat_lvl_mask_targets, concat_lvl_contour_all_points
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            angle_pred_list = [
                deviation_preds[i][img_id].detach() +
                torch.ones(deviation_preds[i][img_id].shape,
                           requires_grad=False).cuda() * math.pi / 18
                for i in range(num_levels)
            ]
