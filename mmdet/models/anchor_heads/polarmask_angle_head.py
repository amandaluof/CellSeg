...
class PolarMask_Angle_Head(nn.Module):
    def __init__(self,
                 num_classes,
                 in_channels,
                 feat_channels=256,
                 ...
                 loss_angle_sum=dict(type='SmoothL1Loss',
                                     beta=1.0,
                                     loss_weight=1.0),
                 loss_angle_disperse=dict({}),
        ...
        super(PolarMask_Angle_Head, self).__init__()
        ...
        # add by amd
        self.loss_angle_sum = build_loss(loss_angle_sum)
        self.loss_angle_disperse = build_loss(loss_angle_disperse)
    ...
    def forward_single(self, x, scale_bbox, scale_mask):
        ...
        angle_pred = self.polar_angle(angle_feat).float().exp()
    ...
    def loss(self,
        ...
        # 根据pred先累加，再在flatten_contour_all_points中选出对应的位置作为极径的真值
        # 可以根据theta对应的值选出相邻两个角度的极径，进行线性插值
        pred_delta_angle = flatten_angle_preds.clone().detach()
        pred_delta_angle = (pred_delta_angle /
                            np.pi) * 180  # 将原有的0-2pi转换为0-360
        ...
        weight_right_contour = (
            pred_inds.flatten() - y_inds_left*interval
        ) / interval  #  # [num_pixels*36],分配左右权重，离左边的点近则右边的点权重大，
        ...
        if num_pos > 0:
            ...
            if (pos_mask_targets == 0).all():
                import pdb
                pdb.set_trace()
            loss_mask = self.loss_mask(pos_angle_preds,
                                       pos_mask_preds,
                                       pos_mask_targets,
                                       weight=pos_centerness_targets,
                                       avg_factor=pos_centerness_targets.sum())
            loss_dict['loss_mask'] = loss_mask
            angle_regular = torch.ones_like(pos_angle_preds.sum(axis=1),
                                            requires_grad=False) * math.pi * 2
            angle_regular = angle_regular.cuda()

            # 约束预测的delta_angle累计和为2pi
            loss_angle_sum = self.loss_angle_sum(
                pos_angle_preds,
                angle_regular,
                weight=pos_centerness_targets,
                avg_factor=pos_centerness_targets.sum())
            loss_dict['loss_angle_sum'] = loss_angle_sum

            # 约束预测的delta_angle标准差尽量小
            loss_angle_disperse = self.loss_angle_disperse(
                pos_angle_preds,
                weight=pos_centerness_targets,
                avg_factor=pos_centerness_targets.sum())
            loss_dict['loss_angle_disperse'] = loss_angle_disperse
            ...
            loss_angle_sum = pos_angle_preds.sum()
            loss_angle_disperse = torch.zeros(pos_angle_preds.sum().shape,
                                              requires_grad=True)
