        ...
        if not self.predict_refine_first:
            ...
            if self.fuse_intermediate_feat:
                ...
                refine_feat = self.compute_offset_feature(
                    mask_refine_feat, offset1_detach,
                    padding_mode='border')  # (N,36,C,H,W)
            # print(refine_feat.mean())
            # refine_feat2 = self.compute_offset_feature(
            #     mask_refine_feat, offset1_detach,
            #     padding_mode='border')  # (N,36,C,H,W)
            # print(refine_feat2.mean())
            # print((refine_feat == refine_feat2).all())
        ...
        if num_pos > 0:
            ...
            if self.loss_pts_bbox_init is not None:
                pos_bbox_init = pts2bbox(
                    pos_mask_init)  # bbox左上右下点的相对坐标/偏移 (x1,y1,x2,y2)
            ...
            if self.loss_pts_bbox_refine is not None:
                pos_bbox_refine = pts2bbox(
                    pos_mask_refine)  # bbox左上右下点的相对坐标/偏移 (x1,y1,x2,y2) (N,4)
...
def pts2bbox(pts):
    ...
    Args:
        pts (Tensor): Shape (n, 36, 2) 表示点相较中心的偏移，有正有负
    
    Returns:
        Tensor: bbox transformed from pts (x1, y1, x2, y2)
    '''
    x1 = pts[:, :, 0].min(dim=1)[0].abs()[:, None]  # (n,1)
    y1 = pts[:, :, 1].min(dim=1)[0].abs()[:, None]
    x2 = pts[:, :, 0].max(dim=1)[0].abs()[:, None]
    y2 = pts[:, :, 1].max(dim=1)[0].abs()[:, None]
    pst_left_top_right_down = torch.cat([x1, y1, x2, y2], dim=1)
